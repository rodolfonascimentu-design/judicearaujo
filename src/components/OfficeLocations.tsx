...
const offices = [
  { name: "Zona Sul - RJ", address: "Rua Aníbal de Mendonça, 27 / 5º andar - Ipanema", cep: "22.410-050", phones: [{ label: "Tel.", number: "+55 (21) 2540.9999", href: "tel:+552125409999" }], whatsapp: { number: "+55 (21) 99559.2196", href: "https://wa.me/5521995592196" }, email: "zonasul@judicearaujo.com.br", hours: ["Segunda à Sexta: 9h às 18:40h", "Sábado: 9:30h às 14:30h", "Domingo e feriados: Fechada com plantão remoto"], image: officeZonaSul },
  { name: "Itaipava - Petrópolis", address: "Estrada União Indústria, 9.450 - Itaipava", cep: "25.730-735", phones: [{ label: "Tel.", number: "+55 (24) 2222.0382", href: "tel:+552422220382" }], whatsapp: { number: "+55 (21) 99967.3830", href: "https://wa.me/5521999673830" }, email: "itaipava@judicearaujo.com.br", hours: ["Segunda à Sexta: 9h às 18h", "Sábado: 9h às 13h", "Domingo e feriados: Fechada com plantão remoto"], image: officeSerra },
  { name: "Barra & Lançamentos - RJ", address: "Av. das Américas, 500/Bl:2/209 - DownTown", cep: "22.640-100", phones: [{ label: "Barra", number: "+55 (21) 99967.3830", href: "tel:+5521999673830" }, { label: "Lançamentos", number: "+55 (21) 99511.3331", href: "tel:+5521995113331" }], whatsapp: null, email: "lancamentos@judicearaujo.com.br", hours: ["Segunda à Sexta: 9h às 18h", "Sábado: 9h às 13h", "Domingo e feriados: Fechada com plantão remoto"], image: officeBarra },
];
...
const OfficeLocations = () => {
  ...
  const goTo = (dir: "prev" | "next") => {
    setActiveIndex((prev) => dir === "next" ? (prev + 1) % offices.length : (prev - 1 + offices.length) % offices.length);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-0 min-h-[500px]">
          <div className="flex flex-col justify-between pr-0 lg:pr-12">
            ...
            <div className="flex items-center gap-4 pt-8">
              ...
              <span className="text-xs tracking-[0.2em] text-muted-foreground font-light">{String(activeIndex + 1).padStart(2, "0")} / {String(offices.length).padStart(2, "0")}</span>
              <button onClick={() => goTo("next")} className="w-10 h-10 rounded-full border border-border/50 flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-foreground transition-all duration-300" aria-label="Próximo escritório"><ChevronRight size={18} /></button>
            </div>
          </div>

          <div className="relative overflow-hidden rounded-[4px] mt-8 lg:mt-0 aspect-[4/3] lg:aspect-auto">
            <AnimatePresence mode="wait">
              <motion.img key={activeIndex} src={active.image} alt={`Escritório ${active.name}`} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.6, ease: [0.25, 0.1, 0.25, 1] }} className="w-full h-full object-cover absolute inset-0" loading="lazy" />
            </AnimatePresence>
          </div>
        </div>
      </div>
