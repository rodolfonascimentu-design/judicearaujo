...
const farms = [
  ...
  { image: property5, title: "Retiro Tropical · Miguel Pereira", area: 15000, bedrooms: 7, parking: 5 },
  { image: property6, title: "Fazenda à Beira do Rio · Vassouras", area: 20000, bedrooms: 6, parking: 6 },
];
...
const FarmsGallery = () => {
  ...
  const PropertyInfo = ({ item, className = "", dark = false }: { item: typeof farms[0]; className?: string; dark?: boolean }) => (
    <div className={`flex items-center gap-4 ${dark ? 'text-muted-foreground' : 'text-cream/60'} ${className}`}>
      <span className="flex items-center gap-1.5 text-xs font-sans font-light">
        <Maximize className="w-3.5 h-3.5" /> {item.area.toLocaleString()}m²
      </span>
      <span className="flex items-center gap-1.5 text-xs font-sans font-light">
