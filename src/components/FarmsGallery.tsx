    ...
    <section ref={sectionRef} className="py-32 lg:py-44 bg-[#FDFDFD] overflow-hidden">
      <motion.div style={{ opacity }} className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="text-center mb-16 lg:mb-24">
          <p className="font-sans text-[10px] tracking-[0.35em] uppercase text-muted-foreground mb-4 font-medium">{t("farms.subtitle")}</p>
          <h2 className="font-display text-2xl md:text-4xl font-normal text-foreground tracking-[-0.02em]">{t("farms.title")}</h2>
        </div>
        ...
        <div className="relative max-w-6xl mx-auto">
            ...
            <div className="flex-1 min-w-0">
              <div className="relative overflow-hidden rounded-[6px] aspect-[4/3] md:aspect-[16/9] lg:aspect-[2/1]">
                <AnimatePresence mode="wait">
                  <motion.img key={currentIndex} src={current.image} alt={current.title} className="w-full h-full object-cover" initial={{ opacity: 0, scale: 1.02 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.6 }} drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.15} onDragEnd={(_, info) => { if (info.offset.x < -SWIPE_THRESHOLD) goTo("next"); else if (info.offset.x > SWIPE_THRESHOLD) goTo("prev"); }} />
                </AnimatePresence>
                <div className="absolute inset-0 bg-gradient-to-t from-charcoal/40 via-transparent to-transparent pointer-events-none" />
                <motion.div className="absolute bottom-6 left-6 right-6 md:bottom-8 md:left-8 pointer-events-auto" key={`btn-${currentIndex}`} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
          ...
          <motion.div className="mt-6 px-1 md:pl-[calc(2.75rem+1.25rem)]" key={`info-${currentIndex}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.15 }}>
            ...
            <div className="flex items-center gap-4 text-muted-foreground">
              <span className="flex items-center gap-1.5 text-xs font-sans font-normal"><Maximize className="w-3.5 h-3.5" /> {current.area.toLocaleString()}m²</span>
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            <button onClick={() => goTo("prev")} className="md:hidden w-8 h-8 bg-white hover:bg-white shadow-sm rounded-full flex items-center justify-center text-black transition-all duration-300" aria-label="Anterior"><ChevronLeft className="w-4 h-4" /></button>
            <div className="flex gap-2">
              {farms.map((_, i) => (<button key={i} onClick={() => setCurrentIndex(i)} className={`h-1 rounded-full transition-all duration-300 ${i === currentIndex ? "w-6 bg-primary" : "w-1.5 bg-border"}`} aria-label={`Fazenda ${i + 1}`} />))}
            </div>
            <button onClick={() => goTo("next")} className="md:hidden w-8 h-8 bg-white hover:bg-white shadow-sm rounded-full flex items-center justify-center text-black transition-all duration-300" aria-label="Próximo"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
      </motion.div>
    </section>
