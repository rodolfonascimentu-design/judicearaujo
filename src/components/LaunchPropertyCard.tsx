...
interface LaunchPropertyCardProps {
  id: string;
  image: string;
  images?: string[];
  title: string;
...
const LaunchPropertyCard = ({
  ...
  return (
    ...
    >
      ...
      >
        ...
        {tag && (
          <div className="absolute top-3 right-3">
            <span className="bg-charcoal/60 backdrop-blur-md text-cream text-[9px] font-sans font-medium tracking-[0.15em] uppercase px-2 py-1 rounded-[3px]">
              {tag}
            </span>
          </div>
        )}
        ...
        {/* Video indicator */}
        {hasVideo && (
          <div className="absolute top-3 left-3 w-7 h-7 rounded-full bg-charcoal/30 backdrop-blur-md flex items-center justify-center">
      ...
      {/* Info */}
      <p className="font-sans text-[10px] tracking-[0.12em] uppercase text-muted-foreground mb-2 leading-relaxed">
        {types}
      </p>
      ...
      <p className="font-sans text-xs text-muted-foreground mb-3 font-light">
        {bedroomsRange} · {bathroomsRange} · {areaRange} · {parkingRange}
      </p>
...
export default LaunchPropertyCard;
